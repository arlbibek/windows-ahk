...
| <kbd>CapsLock</kbd> & <kbd>7</kbd>                                | Transform selected text to lower case (eg. `Hello, Word!` to `hello, word!`) ⚠ _for some reason while changing the case of text with multiple lines, each new line (\n or `n) is sent twice_ |
